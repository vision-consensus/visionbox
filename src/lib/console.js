...
function Console(tasks, options) {
  ...
  try {
    this.visionWrap = VisionWrap(options.networks[options.network], {
      verify: true,
      log: options.log
    })
  } catch (err) {
  ...
  // this.visionWrap.setHttpProvider(options.provider);
...
Console.prototype.start = function (callback) {
  ...
  this.provision(function (err, abstractions) {
    ...
    self.repl.start({
      prompt: 'visionbox(' + self.options.network + ')> ',
      context: {
        visionWrap: self.visionWrap,
      },
